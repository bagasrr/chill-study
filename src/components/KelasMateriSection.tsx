  ...
  return (
    <div className="grid grid-cols-1 gap-4">
      {materiList.map((materi: any) => (
        <PricingCard key={materi.id} id={materi.id} title={materi.title} price={materi.price} link={`/dashboard/kelas/${kelasNama}/materi/${materi.id}`} canAccess={materi.canAccess} />
      ))}
    </div>
  );
};
